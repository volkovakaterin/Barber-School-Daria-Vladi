...
const chatIDDev = "300712588"; //? Katya DEV chatID
...
const requestListener = function (req, res) {
  ...
  const messagesRequest = (address) => {
    ...
    if (address === "/send_registration_form") { //отправка формы записи на курс
      ...
      if (methodReq == "OPTIONS") {
        ...
        req.on("end", function () {
          ...
          bot.sendMessage(chatIDDev, message);
          res.writeHead(200);
          res.end(messages);
        });
      }
    } else if (address === "/open_credit_form") { //открытие формы заявки на рассрочку
      if (methodReq == "OPTIONS") {
        console.log("options");
        res.setHeader("200", "OK");
        res.setHeader(
        ...
        req.on("end", function () {
          let message;
          message = `Форма заявки на рассрочку на курс ${
            JSON.parse(data).name
          }, стасус: ${JSON.parse(data).status}`;
          bot.on("message", (msg) => {
            var Hi = "hi";
            if (msg.text.toString().toLowerCase().indexOf(Hi) === 0) {
              bot.sendMessage(msg.chat.id, "Hello dear user");
